...
{-
   ...
   y otras son parciales.
      ...
      null :: [a] Ver si la lista es vacía (T)

      head :: [a] -> a Primer elemento de la lista (P)

      last :: [a] -> a Último elemento de la lista (P)

      tail :: [a] -> [a] Lista sin el primer elemento (P)

      init :: [a] -> [a] Lista sin el último elemento (P)
 
      take :: Int -> [a] -> [a] Toma los n primeros valores (T)

      drop :: Int -> [a] -> [a] Quita los n primeros valores (T)
      ...
      elem :: Eq a => a -> [a] -> Bool  Ver si es elemento de la lista

      notElem :: Eq a => a -> [a] -> Bool Ver si no es elemento de la lista

      reverse :: [a] -> [a] Lista en sentido contrario

      (++) :: [a] -> [a] -> [a] Concatenación (T)
-}
...
-- Para probar quickCheck en una sentencia polimórfica se comprueba en (), 
-- para probarlo bien debemos forzar un tipo concreto para probarla.

-- Forzamos el tipo de esta manera: quickCheck (prop_disparate :: Int -> [Int] -> Bool)
...
último [] = error "último: lista vacía"
último [x] = x
...
longitud' xs = longi xs 0
   where
      longi [] ac = ac
      longi (_:xs) ac = longi (xs) (ac+1)
...
inversa' :: [a] -> [a] -- predefinida como reverse
inversa' xs = revers xs []
  where
     revers [] ac = ac
     revers (x:xs) ac = revers xs (x:ac)



sumar' :: [Int] -> Int
sumar' xs = sumarAc xs 0 -- [] sol caso base
   where
     sumarAc[] ac = ac
