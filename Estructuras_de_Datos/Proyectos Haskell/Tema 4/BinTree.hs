...
isElemB y (NodeB x l r) = (x==y) || isElemB y l || isElemB y r
...
atLevelB i EmptyB = []
atLevelB i (NodeB x l r) | i == 0 = [x]
                         | otherwise = atLevelB (i-1) l ++ atLevelB (i-1) r

{-
atLevelB 0 (NodeB x _ _) = [x]
atLevelB i (NodeB _ l r) = atLevelB (i-1) l ++ atLevel (i-1) r
atLevel _ _ = []
-}
...
pathsToB _ EmptyB = []
pathsToB x (NodeB y l r) | x == y = [[y]]
                         | otherwise = map (y:) (pathsToB x l ++ pathsToB x r)

{-
pathsToB _ EmptyB = []
pathsToB num (NodeB x l r) | num == x = [[x]]
                           | (isElemB num l) && (isElemB num r) =  [[x] ++ concat(pathsToB num l)] ++ [[x] ++ concat(pathsToB num r)]
                           | (isElemB num l) = [[x] ++ concat(pathsToB num l)]
                           | (isElemB num r) = [[x] ++ concat(pathsToB num r)]
                           | otherwise = []
-}
...
pathsToB2 _ EmptyB = []
pathsToB2 x (NodeB y l r) | x == y = [[x]] ++ mapear
                          | otherwise = mapear
          where 
            mapear = map (y:) (pathsToB x l ++ pathsToB x r)
