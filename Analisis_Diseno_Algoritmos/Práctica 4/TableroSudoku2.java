...
public class TableroSudoku implements Cloneable {
	...
	protected boolean estaEnSubtablero(int fila, int columna, int valor) {
		// A completar por el alumno
		int fi = fila - (fila%3);
		int co = columna - (columna%3);
		boolean encontrado = false;
		int aux=co;int maxCo=co+3;int maxFi=fi+3;
		while(fi < maxFi && !encontrado) {
			co=aux;
			while(co < maxCo && !encontrado) {
	...
	protected void resolverTodos(List<TableroSudoku> soluciones, int fila, int columna) {
		...
		if(numeroDeLibres()==0) {
			...
			if(estaLibre(fila,columna)) {//intentamos colocar
				for(int numAColocar=1;numAColocar<=9;numAColocar++) {
					if(sePuedePonerEn(fila,columna,numAColocar)) {
						celdas[fila][columna]=numAColocar;
						int col;
						int fil;
						if(columna==8) {
							col=0;
			...
			}else {//Si no esta libre pasamos a la sig celda
				if(columna==8) {
					columna=0;
					fila++;
				}else {
