...
% Graficar los valores objetivo
figure;
plot(Z); % Graficamos Z usando un scatter para visualizar cada punto objetivo
title('Valores Objetivos del Conjunto de Patrones');
xlabel('Patrón');
ylabel('Valor Objetivo');

% Calcular el rango de la función a aprender
min_val = min(Z); % Valor mínimo de Z
max_val = max(Z); % Valor máximo de Z
disp(['Rango de la función a aprender: [', num2str(min_val), ', ', num2str(max_val), ']']);
...
plot(250 , MSEAcumuladoTest_vector(250), 'or')
